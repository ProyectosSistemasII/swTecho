...
namespace Capa_Datos
{
    public class _Reportes
    {
        ...
        public DataTable SalidasInsumos(int idSalida)
        {
            ...
            try
            {
                string consulta = "SELECT Concat(Nombres,' ',Apellidos) AS Voluntario, DATE_FORMAT(FechaSalida, '%d/%m/%Y') AS FechaSalida, Descripcion, UserName, Cantidad, Alimentos.Nombre AS Alimento, CONCAT(Alimentos.Rango, ' ', Alimentos.AnioCaducidad) AS AnioCaducidad, Presentacion.Nombre AS Presentacion FROM Salida INNER JOIN Usuarios ON Usuarios.idUsuarios = Salida.Usuarios_idUsuarios INNER JOIN DetalleSalida ON DetalleSalida.Salida_idSalida = Salida.idSalida INNER JOIN Alimentos ON Alimentos.idAlimentos = DetalleSalida.Alimentos_idAlimentos INNER JOIN Presentacion ON Presentacion.idPresentacion = Alimentos.Presentacion_idPresentacion INNER JOIN Voluntarios ON Voluntarios.idVoluntarios = Salida.Voluntarios_idVoluntarios WHERE Salida.idSalida = @idSalida";
                comando = new MySqlCommand(consulta, _conexion);
         ...
         */
        public DataTable SalidasHerramientas(int idPrestamo)
        {
            MySqlCommand comando = null;
            try
            {
                string consulta = "SELECT Concat(Nombres,' ',Apellidos) AS Voluntario, DATE_FORMAT(FechaPrestamo, '%d/%m/%Y') AS FechaPrestamo, UserName, DetallePrestamo.CantidadPrestada, Herramientas.Nombre AS Herramienta, Observaciones FROM Prestamo INNER JOIN Usuarios ON Usuarios.idUsuarios = Prestamo.Usuarios_idUsuarios INNER JOIN DetallePrestamo ON DetallePrestamo.Prestamo_idPrestamo = Prestamo.idPrestamo INNER JOIN Voluntarios ON Voluntarios.idVoluntarios = Prestamo.Voluntarios_idVoluntarios INNER JOIN Herramientas ON Herramientas.idHerramientas = Detalleprestamo.Herramientas_idHerramientas WHERE Prestamo.idPrestamo = @idPrestamo;";
                comando = new MySqlCommand(consulta, _conexion);
