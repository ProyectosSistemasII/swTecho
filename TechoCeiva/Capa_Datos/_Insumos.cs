...
namespace Capa_Datos
{
    public class _Insumos
    {
        ...
        public DateTime AnioCaducidad { get; set; }
        ...
        public _Insumos(int _idAlimentos,String _nombre, int _existencia, String _rango, DateTime _anioCaducidad, Boolean _activo, int _presentacion)
        {
             ...
             public List<_Insumos> _Obtener_I()
        {
            ...
            for (int i = 0; i < _tabla.Rows.Count; i++)
            {
                ...
                _Insumos _insumos = new _Insumos(Convert.ToInt32(_row["idAlimentos"]),Convert.ToString(_row["Nombre"]), Convert.ToInt32(_row["Existencia"]), Convert.ToString(_row["Rango"]), Convert.ToDateTime(_row["AnioCaducidad"]), Convert.ToBoolean(_row["Activo"]), Convert.ToInt32(_row["Presentacion_idPresentacion"]));
        ...
        public void _Modificar(int _id, int _cantidad)
        {
            if (this._errores.Count == 0)
            {
                string query = "UPDATE Alimentos SET Existencia = @Existencia WHERE idHerramientas = " + _id;
                MySqlCommand _comando = new MySqlCommand(query, _conexion);
