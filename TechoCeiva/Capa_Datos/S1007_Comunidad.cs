...
namespace Capa_Datos
{
    public class S1007_Comunidad
    {
        ...
        public DataTable GenerarReporte(int comunidad)
        {
            try
            {
                string consulta = "SELECT NoInteresa, FaltaInformacion, FaltaTiempo,CompromisoFamiliar, Otro,NSNR    FROM S1007_com   inner join S10_Com on S1007_com_idS1007_com = idS1007_Com inner join Encuestas on Comunidad_idComunidad = 1 and Encuestas_idEncuestas = idencuestas Order by NoInteresa";
                MySqlCommand comando = new MySqlCommand(consulta, conex);
